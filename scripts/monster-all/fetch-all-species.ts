...
async function fetchAndSaveSpecies(id: number): Promise<void> {
  let attempt = 0;
  ...
  while (attempt < MAX_RETRY) {
    try {
      ...
      attempt++;
      console.error(`Failed to fetch species ${id} (attempt ${attempt}):`, error);

      if (attempt >= MAX_RETRY) {
        console.error(`Giving up on species ${id} after ${MAX_RETRY} attempts.`);
        return;
      }
...
export async function main(): Promise<void> {
  ...
  console.log(`Total species to fetch: ${ids.length}`);
  await fs.mkdir(OUTPUT_DIR, { recursive: true });

  const limit = pLimit(CONCURRENCY);
  const tasks = ids.map(id => limit(() => fetchAndSaveSpecies(id)));
